...
캔버스에 떠 있는 프레임 이름 라벨을 클릭 한 번으로 껐다 켜는 Figma용 무료 플러그인입니다.

![이름 숨기기 버튼을 누르면 캔버스의 프레임 이름 라벨이 사라지고, 다시 누르면 돌아옵니다](assets/cover.gif)
...
파일에서 플러그인이 명령을 한 번 실행하고 나면, 오른쪽 패널 아래쪽 **Tools**에 **Hide/Show Frame Name** 버튼이 생깁니다. 아무것도 선택하지 않았을 때, 그리고 플러그인을 실행한 페이지에서 프레임·섹션·컴포넌트를 선택했을 때 보입니다. 나중에 만든 프레임에는 그 페이지에서 플러그인을 다시 실행할 때 붙습니다. 인스턴스 안의 레이어에는 붙지 않습니다. 메뉴를 열지 않고 이 버튼으로 이름을 숨기고 되돌릴 수 있습니다. 버튼은 적용 범위 설정을 따르므로, 범위를 "선택한 레이어"로 두지 않으면 페이지 전체를 바꿉니다.

버튼은 파일에 저장되므로 같은 파일을 여는 사람에게도 보입니다. 레이어에서 버튼을 없애려면 옆의 **−** 버튼을 누르세요. 플러그인이 다시 붙이지 않습니다.
