...
플러그인의 **Frame Name Settings** 화면에서 단축키를 지정하면 위 순서를 그 키에 맞춰 안내합니다. 터미널로 등록할 수도 있습니다.
...
| Frame Name Settings | 설정 화면을 엽니다. |

## 설정

- **적용 범위**: 이 페이지 / 모든 페이지 / 선택한 레이어
- **포함 대상**: 섹션, 컴포넌트, 인스턴스, 중첩된 프레임 (프레임은 항상 포함)
- **언어**: 자동 / English / 한국어
...
- **인스턴스는 기본으로 빠져 있습니다.** 인스턴스 이름을 바꾸면 메인 컴포넌트 이름을 따라가던 연결이 끊어져, 되돌린 뒤에도 수동으로 붙인 이름으로 남습니다.
