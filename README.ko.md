...
파일에서 플러그인이 명령을 한 번 실행하고 나면, 오른쪽 패널 아래쪽 **Tools**에 **Hide/Show Frame Name** 버튼이 생깁니다. 아무것도 선택하지 않았을 때, 그리고 플러그인이 다룬 프레임·섹션·컴포넌트를 선택했을 때 보입니다. 메뉴를 열지 않고 이 버튼으로 이름을 숨기고 되돌릴 수 있습니다. 버튼은 적용 범위 설정을 따르므로, 범위를 "선택한 레이어"로 두지 않으면 페이지 전체를 바꿉니다.
